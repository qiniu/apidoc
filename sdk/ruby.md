...
设置原图保护后，其原图为私有，不能通过 `http://<Domain>/<Key>` 和 `http://<Domain>/<Key>?<Op>/<Params>` 这样的方式来访问。

但是用户可以在原图的 `<Key>` 后面加上“分隔符”，以及相应的水印风格样式的别名来访问打水印后的图片。例如，假设您为用户设定的访问水印图的分隔符为中划线 “-”，那么用户可以通过如下形式来访问打水印后的图片：

    http://<Domain>/<Key>-<Style>

以上 URL 作用效果等价于如下 URL 形式（设置原图保护后该 URL 形式的访问操作会被禁止）：
...
别名（Name） | 规格（Style） | 说明
----------- | ------------ | -------
small.jpg   | imageView/2/w/120/h/90 | 大小为 120x90，不打水印
middle.jpg  | imageView/2/w/440/h/330/watermark/1 | 大小为 440x330，打水印
large.jpg   | imageView/2/w/1280/h/760/watermark/1 | 大小为 1280x760，打水印
